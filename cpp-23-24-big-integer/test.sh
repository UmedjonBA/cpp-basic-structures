...
python3 banned_words_checker.py --solution=$1.hpp --bannded-words=banned_words.json
if [[ ! $? -eq 0 ]]
then 
...
python3 banned_words_checker.py --solution=$1.cpp --bannded-words=banned_words.json
if [[ ! $? -eq 0 ]]
then 
  echo "В вашем коде есть забаненые слова!"
  exit 1
...
clang-tidy -extra-arg=-std=c++17 -quiet $1.hpp
if [[ ! $? -eq 0 ]]
then 
...
clang-tidy -extra-arg=-std=c++17 -quiet $1.cpp
if [[ ! $? -eq 0 ]]
then 
  echo "Проверка clang-tidy не пройдена"
  exit 1
