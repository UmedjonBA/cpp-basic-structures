...
python3 banned_words_checker.py --solution=cpp-23-24-deque.hpp --banned-words=banned_words.json
if [[ ! $? -eq 0 ]]
then
...
clang-tidy -extra-arg=-std=c++20 -quiet cpp-23-24-deque.hpp
if [[ ! $? -eq 0 ]]
then
...
clang-format -i -style='{BasedOnStyle: Google, DerivePointerAlignment: false, PointerAlignment: Left, AlignOperands: true}' cpp-23-24-deque.hpp &&
# git diff --ignore-submodules --color cpp-23-24-deque.hpp > diff &&
# cat diff
# if [[ -s diff ]]
# then
